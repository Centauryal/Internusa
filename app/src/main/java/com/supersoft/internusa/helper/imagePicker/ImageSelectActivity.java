...
public class ImageSelectActivity extends HelperActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        album = intent.getStringExtra(Constant.INTENT_EXTRA_ALBUM);
    ...
    private void toggleSelection(int position) {
        if (!images.get(position).isSelected && countSelected >= Constant.limit) {
            Toast.makeText(
                    getApplicationContext(),
                    String.format(getString(R.string.limit_exceeded), Constant.limit),
                    Toast.LENGTH_SHORT)
                    .show();
            return;
        }
    ...
    private void sendIntent() {
        ...
        intent.putParcelableArrayListExtra(Constant.INTENT_EXTRA_IMAGES, getSelected());
    ...
    private class ImageLoaderRunnable implements Runnable {
        ...
        public void run() {
            ...
            MergeCursor cursor = new MergeCursor(new Cursor[]{
                    getApplicationContext().getContentResolver().query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, projection, MediaStore.Images.Media.BUCKET_DISPLAY_NAME + " =?", new String[]{album}, MediaStore.Images.Media.DATE_ADDED),
                    getApplicationContext().getContentResolver().query(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, projection, MediaStore.Images.Media.BUCKET_DISPLAY_NAME + " =?", new String[]{album}, MediaStore.Images.Media.DATE_ADDED),
                    getApplicationContext().getContentResolver().query(MediaStore.Images.Media.INTERNAL_CONTENT_URI, projection, MediaStore.Images.Media.BUCKET_DISPLAY_NAME + " =?", new String[]{album}, MediaStore.Images.Media.DATE_ADDED),
                    getApplicationContext().getContentResolver().query(MediaStore.Video.Media.INTERNAL_CONTENT_URI, projection, MediaStore.Images.Media.BUCKET_DISPLAY_NAME + " =?", new String[]{album}, MediaStore.Images.Media.DATE_ADDED)
            });
            if (cursor == null) {
            ...
            if (cursor.moveToLast()) {
                do {
                    ...
                    if (file.exists()) {
                        ...
                        String mimeType = getContentResolver().getType(Uri.fromFile(file));
                        if(mimeType == null)
                        {
                        ...
                        temp.add(new Image(id, name, path, isSelected, mimeType));
                    }
            ...
            sendMessage(Constant.FETCH_COMPLETED, tempCountSelected);
        }
    }
    ...
    protected void permissionGranted() {
        sendMessage(Constant.PERMISSION_GRANTED);
    }
